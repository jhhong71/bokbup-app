...
// .wrangler/tmp/pages-0tqPdm/functionsWorker-0.2758668935110513.mjs
...
__name(dailyPicks, "dailyPicks");
__name2(dailyPicks, "dailyPicks");
...
async function onRequest(context) {
  ...
  if (p === "/api/emoticons" && request.method === "GET") {
    ...
    if (idsParam) {
      const items2 = idsParam.split(",").slice(0, 300).map((id) => BY_ID.get(id)).filter(Boolean);
      return json({ items: items2, total: items2.length });
    }
    let items = ALL;
  ...
  if (p === "/api/emoticons/trend" && request.method === "GET") {
    ...
    const items = trend.map((d) => Object.assign({}, d, { hot: (d.hot || 0) + (stats[d.id] || 0) })).sort((a, b) => (b.hot || 0) - (a.hot || 0));
...
// .wrangler/tmp/bundle-1L0Z1I/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1L0Z1I/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.2758668935110513.js.map
